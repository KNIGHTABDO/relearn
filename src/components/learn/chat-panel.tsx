    ...
    <div className={cn("flex h-full flex-col", className)}>
      ...
      {addedContexts.length > 0 && (
        <div className="flex flex-wrap items-center gap-1.5 border-t border-gray-50 px-4 py-2">
          <span className="text-[10px] text-gray-400 uppercase tracking-wider">Context:</span>
          {addedContexts.map((id) => {
            const doc = availableDocs.find((d) => d.id === id);
            return (
              <span key={id} className="flex items-center gap-1 rounded-full bg-gray-100 px-2 py-0.5 text-[10px] text-gray-600">
      ...
      {showAddContext && (
        <div className="border-t border-gray-100 px-4 py-3 animate-fade-in">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-medium text-gray-700">Add files to context</span>
            <button onClick={() => setShowAddContext(false)} className="text-xs text-gray-400 hover:text-gray-600">Done</button>
          </div>
            ...
            {availableDocs.filter((d) => d.id !== documentId).map((doc) => (
              <button
                ...
                className={cn(
                  ...
                  addedContexts.includes(doc.id) ? "bg-gray-100 text-gray-900" : "text-gray-600 hover:bg-gray-50"
                )}
              >
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={cn(
              ...
              showAddContext ? "border-gray-300 bg-gray-100" : "border-gray-200 hover:bg-gray-50"
            )}
            title="Add Context"
          >
            <Plus className="h-3.5 w-3.5 text-gray-500" />
          </button>
          <div className="flex flex-1 items-center gap-2 rounded-full border border-gray-200 bg-white px-4 py-2.5 focus-within:border-gray-300 focus-within:shadow-sm">
            <input
              ref={inputRef}
              type="text"
              value={input}
