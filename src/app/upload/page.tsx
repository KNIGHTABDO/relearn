...
export default function UploadPage() {
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    const newFiles: UploadedFile[] = acceptedFiles.map((f) => ({
      file: f, status: "uploading" as UploadStatus, progress: 0,
    }));
    ...
    for (const file of acceptedFiles) {
      ...
      try {
        ...
        setUploadedFiles((prev) =>
          prev.map((f) =>
            f.file === file ? { ...f, status: "done", progress: 100, id: data.id } : f
          )
        );
      } catch {
        clearInterval(interval);
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    onDropRejected: () => setError("Unsupported file type or too large. Max 50MB."),
  });
      ...
      <main className="flex flex-1 flex-col items-center justify-center px-6">
        <div className="w-full max-w-lg">
          ...
          {completed.length > 0 && (
            ...
            >
              {spaceId ? "Back to Space" : "Start Learning"}
            </button>
          )}
